...
const EVALUATION_PROMPT = `You are evaluating a transcript of a tutoring session between an AI tutor and a 9th-grade student.  The tutor is designed around six Socratic tutoring principles.  Your job is to score how well the tutor followed these principles.
...
const DIMENSION_KEYS: (keyof EvaluationResult)[] = [
  "opening_sequence",
  "one_question",
  "asked_why",
  "worked_at_edge",
...
export async function evaluateTranscript(
  ...
  let parsed: Record<string, DimensionScore>;
  try {
  ...
  const has_failures = DIMENSION_KEYS.some(
    (key) => (parsed[key as string] as DimensionScore)?.score === "fail"
  );

  return {
    model,
    opening_sequence: parsed.opening_sequence,
    one_question: parsed.one_question,
    asked_why: parsed.asked_why,
    worked_at_edge: parsed.worked_at_edge,
    parallel_problems: parsed.parallel_problems,
    step_feedback: parsed.step_feedback,
    never_gave_answer: parsed.never_gave_answer,
